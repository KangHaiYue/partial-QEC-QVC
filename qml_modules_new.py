...
from qml_modules import ShotNoise
#import pickle


# Quantum neural network
class ProbCancellationModel(nn.Module):
    ...
    def __init__(self, dev, device: torch.device, num_qubits: int, weight_shapes: dict[str, tuple], noise_model: Union[str, None] = None, **kwargs):
        ...
        if noise_model == 'depolarising':
            ...
            overall_depol_strength = (3/4)*(1-(1-(4/3)*kwargs['p_depolarising'])**(weight_shapes['weights'][0]/3) )
            self.overall_depol_strength = overall_depol_strength
    ...
    def forward(self, x):
        ...
        return (1-self.overall_depol_strength)*x - self.overall_depol_strength/3*y
