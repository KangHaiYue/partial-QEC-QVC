def test_job(p_depolarising: float) -> None:
    """
    Quantum neural network training/testing with CPU-only parallel backpropagation within each batch using MPI.
    Each batch (size=50) is split among available CPU processes for parallel loss/backward.
    """
    import torch
    ...
    from util_funcs import process_batch_mpi
    ...
    if rank == 0:
        print(f"Using device: {device} (MPI size={size})", flush=True)
    ...
    # Broadcast datasets to all ranks (only indices, not data)
    train_dataset = comm.bcast(train_dataset, root=0)
    ...
    if rank == 0:
        print("Using default.mixed simulator (CPU, MPI parallel)", flush=True)
    ...
    train_subset = Subset(train_dataset, range(epoch_size))
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        target_onehot = torch.zeros(data.size(0), 10, device=device)
        target_onehot.scatter_(1, target.view(-1, 1), 1)

        optimizer.zero_grad()
        
        # Parallel loss/backward within batch
        batch_loss, nan_count = process_batch_mpi(model, data, target_onehot, batch_size, batch_idx, minibatch_size, criterion, comm, rank, size)
        ...
        training_samples.append(trained_images)
        if rank == 0:
            print(f"Batch {batch_idx}: loss={batch_loss:.4f}, nan_count={nan_count}, trained_images={trained_images}", flush=True)
    ...
    if rank == 0:
        ...
        with open(f'noisy_QNN_test/QVC50_10q_encoded_50batch_15000epoch_0005lr_depol_{p_depolarising:.2e}_mpi.pkl', 'wb') as file:
            import pickle
            pickle.dump(results, file, protocol=pickle.HIGHEST_PROTOCOL)
        print("Training complete. Results saved.", flush=True)
    return
